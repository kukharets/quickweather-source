import React from 'react';
import { connect } from 'react-redux/es/alternate-renderers';
import { Typography } from '@material-ui/core';
import {
  Whatsapp,
  Telegram,
  Google,
  Facebook,
  Mail,
  Twitter,
} from 'react-social-sharing';
import MenuIcon from '../assets/menu.svg';
import ShareIcon from '../assets/share.svg';

class ExpandMenu extends React.Component {
  state = {
    menuOpenState: false,
    shareOpenState: false,
  };

  switchMenuOpenState = () => {
    const {
      state: { menuOpenState, shareOpenState },
    } = this;
    this.setState({
      menuOpenState: !menuOpenState,
      shareOpenState:
        shareOpenState && !menuOpenState ? !shareOpenState : shareOpenState,
    });
  };

  switchShareOpenState = () => {
    const {
      state: { shareOpenState, menuOpenState },
    } = this;
    this.setState({
      shareOpenState: !shareOpenState,
      menuOpenState:
        menuOpenState && !shareOpenState ? !menuOpenState : menuOpenState,
    });
  };

  render() {
    const {
      state: { menuOpenState, shareOpenState },
      props: { isDesktop, mainText },
    } = this;
    return (
      <div className="menu-buttons">
        <ShareIcon
          onClick={this.switchShareOpenState}
          className={`header-button${isDesktop ? '_desktop' : ''}`}
        />
        <MenuIcon
          onClick={this.switchMenuOpenState}
          className={`header-button${isDesktop ? '_desktop' : ''}`}
        />

        {shareOpenState && (
          <div className="share-block">
            <Typography className="share-icon" inline color="dark">
              Share weather in <b>{mainText}</b>
            </Typography>
            <div className="share-icons">
              <Whatsapp link={`${window.location.href}`} />
              <Telegram link={`${window.location.href}`} />
              <Twitter link={`${window.location.href}`} />
              <Google link={`${window.location.href}`} />
              <Facebook link={`${window.location.href}`} />
              <Mail link={`${window.location.href}`} />
            </div>
          </div>
        )}
        {menuOpenState && (
          <div className="menu-list">
            <a href="https://github.com/kukharets/quickweather-source">
              <div className="menu-item">
                <Typography className="clickable-item" inline color="dark">
                  View source code
                </Typography>
                <span className="svg-background github-icon social-icon clickable-item" />
              </div>
            </a>
            <a href="https://www.upwork.com/freelancers/~0169f6733a03d07859">
              <div className="menu-item">
                <Typography className="clickable-item" inline color="dark">
                  Visit author UpWork
                </Typography>
                <span className="svg-background upwork-icon social-icon clickable-item" />
              </div>
            </a>
            <a href="https://www.linkedin.com/in/taras-kukharets/">
              <div className="menu-item last-menu-item">
                <Typography className="clickable-item" inline color="dark">
                  Visit author LinkedIn
                </Typography>
                <span className="svg-background linkedin-icon social-icon clickable-item" />
              </div>
            </a>
          </div>
        )}
      </div>
    );
  }
}

const mapStateToProps = ({ markup, basic }) => {
  const {
    device: { isDesktop },
  } = markup;
  const {
    selectedPlace: { structured_formatting: { main_text: mainText } = {} },
  } = basic;
  return {
    isDesktop,
    mainText,
  };
};

export default connect(
  mapStateToProps,
  {},
)(ExpandMenu);
